...
class Auth {
  constructor(dataDir) {
    ...
    this.bearerFails = new Map(); // ip -> { count, reset } - failed Authorization: Bearer guesses
  ...
  // Bearer token guesses are checked on every API request, unlike /api/login, so they need
  // their own limiter that only counts *failed* attempts (a valid token must never be throttled).
  bearerBlocked(ip) {
    const a = this.bearerFails.get(ip);
    return Boolean(a && a.reset > Date.now() && a.count > 10);
  }

  recordBearerFailure(ip) {
    const now = Date.now();
    const a = this.bearerFails.get(ip);
    if (!a || a.reset < now) { this.bearerFails.set(ip, { count: 1, reset: now + 60e3 }); return; }
    a.count++;
  }
  ...
  cleanup() {
    ...
    for (const [k, a] of this.bearerFails) if (a.reset < now) this.bearerFails.delete(k);
  ...
  // Returns the signed-in user, or null. `ip` is used to rate-limit Bearer token guesses,
  // since (unlike /api/login) any authenticated route accepts an Authorization header.
  user(req, ip) {
    const header = req.headers.authorization || '';
    if (header.startsWith('Bearer ')) {
      if (ip && this.bearerBlocked(ip)) return null;
      const ok = this.check(header.slice(7));
      if (!ok && ip) this.recordBearerFailure(ip);
      return ok ? { name: 'API token', method: 'token' } : null;
    }
